...
//arrive.js v2.2.0 https://github.com/uzairfarooq/arrive MIT licensed Copyright (c) 2014-2015 Uzair Farooq
...
;(function() {
  ...
  controller.init({
    siteName: "Embedded HTML5",
  });
})();

},{"../modules/SKLog.js":4,"BaseController":3,"jquery":2}],2:[function(require,module,exports){
(function (global){
...
;(function() {
  ...
  var audioSourceDict_ = {};
  var globallyPaused_ = false;
  var disabled_ = false;
  var playCount_ = 0;
  var pauseCount_ = 0;
  var noContentCount_ = 0;

  var EXT_CLASS_PREFIX = "mutetabid_";

  function getUniqueClassName_() {
    while (true) {
      var className = EXT_CLASS_PREFIX + Math.floor(Math.random() * 10000001);
      if ($("." + className).length === 0)
        return className;
    }
  }

  function getClassesWithPrefix_(elem, strClassBegin) {
    if ((elem === null) || (elem.classList.length === 0))
      return [];
    var classListArray = Array.prototype.slice.call(elem.classList, 0);
    return classListArray.filter(function(className) {
      return className.startsWith(strClassBegin);
    });
  };

  function getMuteTabId(elem) {
//    sk_log("getMuteTabId", elem);
    var className = getGeneratedClassName_(elem);
    return className;
  }

  // Returns the first unique class name that start with EXT_CLASS_PREFIX for the element.
  // If none found, creates one and assigns it to the class
  function getGeneratedClassName_(elem) {
    var className = null;
    var generatedClassNames = getClassesWithPrefix_(elem, EXT_CLASS_PREFIX);
//    sk_log("generatedClassNames", generatedClassNames);
//    sk_log(elem);
    if (generatedClassNames.length > 0)
      className = generatedClassNames[0];
    else {
      className = getUniqueClassName_();
      elem.classList.add(className);
    }
    return className;
  }

  // TODO: look at checking (elems[i].seeking === true)
  function getAudibleAndInaudibleCounts(elems) {
    var obj = {};
    obj.audibleCount = 0;
    obj.inaudibleCount = 0;
    obj.noContentCount = 0;
    ...
    for (var i = 0; i < elemsLength; i++) {
      var mutetabId = getMuteTabId(elems[i]);
      audioSourceDict_[mutetabId] = audioSourceDict_[mutetabId] || {};
      var audioSourceInfo = audioSourceDict_[mutetabId];

      var elemProperties = {
        paused: elems[i].paused,
        seeking: elems[i].seeking,
        muted: elems[i].muted,
        wasUnmuted: audioSourceInfo.wasUnmuted || false
      };
      sk_log("elem", elems[i]);
      sk_log("elem properties", elemProperties);

      if (elems[i].webkitAudioDecodedByteCount > 0) {
        if ((((elems[i].paused === false) || (elems[i].seeking)) &&
            ((elems[i].muted === false) || (audioSourceInfo.wasUnmuted || false) === true))) {
          sk_log("isaudible++", elems[i]);
          obj.audibleCount++;
        } else {
          obj.inaudibleCount++;
        }
      ...
      if ((elems[i].muted === false) && (elems[i].paused === false))
        audioSourceInfo.wasUnmuted = true;
    }
    sk_log("getaudibleandinaudiblecounts", obj);
    return obj;
  }

  function clearPaused() {
    var keys = Object.keys(audioSourceDict_);
    keys.forEach(function(key) {
      delete audioSourceDict_[key].savePaused;
    })
  }

  function playAll(unduckOnly) {
    playAllForSelector($("video"));
    playAllForSelector($("audio"));
    globallyPaused_ = false;
  }

  function playAllForSelector($elems) {
    for (var i = 0; i < $elems.length; i++) {
      var audioSourceInfo = audioSourceDict_[getMuteTabId($elems[i])] || {};
      if ((globallyPaused_ === false) || (audioSourceInfo.savePaused || false)) {
        sk_log("for playall: savePaused", audioSourceInfo.savePaused);
        sk_log($elems[i]);

        delete audioSourceInfo.savePaused;
        $elems[i].muted = false;
        $elems[i].play();
      }
    }
  ...
  function getSavePausedCount() {
    var savePausedCount = 0;
    Object.keys(audioSourceDict_).forEach(function(key) {
      if (audioSourceDict_[key].savePaused)
        savePausedCount++;
    });

    return savePausedCount;
  }

  function pauseAll() {
    clearPaused();
    pauseAllForSelector($("video"));
    pauseAllForSelector($("audio"));
    globallyPaused_ = true;
  }

  function pauseAllForSelector($elems) {
    for (var i = 0; i < $elems.length; i++) {
      if (!$elems[i].paused) {
        $elems[i].pause();
        var id = getMuteTabId($elems[i]);
        audioSourceDict_[id] = audioSourceDict_[id] || {};
        audioSourceDict_[id].savePaused = true; // so we can unpause just what was paused later
        sk_log("savePaused", $elems[i]);
        sk_log("pause", $elems[i]);
      }
    }
  }
  ...
  BaseController.prototype.init = function(options) {
    ...
    BaseController.prototype.playPause = function() {
      sk_log("__playPause__");
      var videoCounts = getAudibleAndInaudibleCounts($("video"));
      var audioCounts = getAudibleAndInaudibleCounts($("audio"));
      var counts = {
        audibleCount: videoCounts.audibleCount + audioCounts.audibleCount,
        inaudibleCount: videoCounts.inaudibleCount + audioCounts.inaudibleCount,
        noContentCount: videoCounts.noContentCount + audioCounts.noContentCount
      }
      sk_log("frame" + BaseController.prototype.randomFrameId, counts);
      if (counts["audibleCount"] > 0)
        pauseAll();
      else
        playAll();
    ...
    BaseController.prototype.play = function() {
      sk_log("__play__");
      playAll();
    };

    BaseController.prototype.pause = function() {
      sk_log("__pause__");
      pauseAll();
    };
  };
   ...
   * Method rewritten for MuteTab-new to find <audio> and <video> elements
   * @return {Boolean} true if site is currently playing
   */
  BaseController.prototype.isPlaying = function() {
    // Treat it as playing if the autoplay countdown is shown (for YouTube)
    var $ytpUpNext = $(".ytp-upnext");
    if ($ytpUpNext.length && (!$ytpUpNext[0].classList.contains("ytp-upnext-autoplay-paused"))) {
      isPlaying = true;
      return isPlaying;
    }
    ...
    if (counts.audibleCount > 0)
      isPlaying = true;
    else if (counts.inaudibleCount > 0)
      isPlaying = false;

    sk_log("isplaying = " + isPlaying);

    playCount_ = counts.audibleCount;
    pauseCount_ = counts.inaudibleCount;
    noContentCount_ = counts.noContentCount;
   ...
   */
  BaseController.prototype.updatePlayerState = function(e) {
    if (disabled_)
      return;
    sk_log("updatePlayerState", e)
    if(this.checkPlayer) this.checkPlayer();

    var newState = this.getStateData();
    newState.fromDefaultController = true;
  ...
  BaseController.prototype.getStateData = function() {
    return {
      ...
      noContentCount: noContentCount_,
      savePausedCount: getSavePausedCount(),
      siteName: this.siteName,
      frameId: BaseController.prototype.randomFrameId,
      supportsSeparatePlayPause: true
    };
  };
  ...
  BaseController.prototype.doRequest = function(request, sender, response) {
    ...
    sk_log("request", request);

    if (request.customOnly || false) {
      if (disabled_ === false) {
        sk_log("DISABLED!!! (via request)");
      }
      disabled_ = true; // TODO: also stop event listeners since they won't be used
      return;
    }

    if (request.hasOwnProperty("intendedCommand"))
      request.action = request.intendedCommand;
    ...
    if (typeof request !== "undefined") {
      ...
      if(request.action === "play") this.play();
      if(request.action === "pause") this.pause();
  ...
  BaseController.prototype.initEventListeners = function(newThis, $elems) {
    sk_log("_initEventListeners");
    sk_log('elemslength: ' + $elems.length.toString());
    sk_log($elems);
    for (var i = 0; i < $elems.length; i++) {
      sk_log(".");
      $elems[i].addEventListener("play", this.updatePlayerState.bind(newThis));
      $elems[i].addEventListener("pause", this.updatePlayerState.bind(newThis));
      $elems[i].addEventListener("volumechange", this.updatePlayerState.bind(newThis));
      $elems[i].addEventListener("loadeddata", this.updatePlayerState.bind(newThis));

//      $elems[i].addEventListener("seeking", this.updatePlayerState.bind(newThis));
    }
  }

  BaseController.prototype.addListenersAndUpdatePlayerState = function() {
    if (disabled_)
      return;
    sk_log("addListenersAndUpdatePlayerState", this);
    BaseController.prototype.initEventListeners(BaseController.prototype.self, $(this));
    BaseController.prototype.updatePlayerState.bind(BaseController.prototype.self)();
  }
  ...
  BaseController.prototype.attachListeners = function() {
    BaseController.prototype.randomFrameId = Math.floor(Math.random() * 10000001);
    ...
    BaseController.prototype.self = this;

    this.initEventListeners(this, $("video"));
    this.initEventListeners(this, $("audio"));

    document.arrive("video", this.addListenersAndUpdatePlayerState);
    document.leave("video", this.addListenersAndUpdatePlayerState);
    document.arrive("audio", this.addListenersAndUpdatePlayerState);
    document.leave("audio", this.addListenersAndUpdatePlayerState);
    sk_log("Attached listeners for ", this);
...
},{"../modules/SKLog.js":4}],4:[function(require,module,exports){
;(function() {
  "use strict";
