...
var mutetabPrefs = mutetabPrefs || {};
var mutetabPrefs_ = function mutetabPrefs_() {
  var self = this;
  var prefs_ = void 0;
  ...
  var getPrimaryDomain = function getPrimaryDomain(domain) {
  ...
  this.initListEditor = function (elem, dict) {
    try {
      ...
      Object.keys(dict).sort(function (a, b) {
        var primary1 = getPrimaryDomain(a);
        var primary2 = getPrimaryDomain(b);
  ...
  var addDomainListEventListeners = function addDomainListEventListeners(elem) {
  ...
  var onAddDomainClick = function onAddDomainClick(e) {
    e.preventDefault();
    var form = $(this).closest("form")[0];
    console.log(form);
    var $listSelect = $(".listSelect", $(form));
    var $urlTextBox = $(".urlTextBox", $(form));
    var domain = getDomain($urlTextBox[0].value);
    if (domain === null) return; // TODO: show better error message?
    ...
    var $options = $("option", $listSelect);
    var matchedNodes = $options.filter(function () {
  ...
  var onRemoveDomainClick = function onRemoveDomainClick(e) {
    e.preventDefault();
    var form = $(this).closest("form")[0];
    console.log(form);
    var $listSelect = $(".listSelect", $(form));
  ...
  var onRawFiltersClick = function onRawFiltersClick(e) {
    e.preventDefault();
    ...
    var form = $(this).closest("form")[0];
    console.log(form);
    var $editRawSection = $(".editRawSection", $(form));
    var $rawTextArea = $(".rawTextArea", $(form));
  ...
  var onImportRawClick = function onImportRawClick(e) {
    e.preventDefault();
  ...
  this.onValidate = function () {
    var minTimeBeforeDuckingStr = $("#enable_min_time_before_ducking")[0].checked ? $("#min_time_before_ducking")[0].value : 0;

    var minTimeBeforeDuckingVal = parseFloat(minTimeBeforeDuckingStr);
    ...
    var minTimeBeforeUnduckingStr = $("#min_time_before_unducking")[0].value;
  ...
  this.onSave = function (e) {
    ...
    if (self.onValidate()) {
      ...
      prefsStore.save(prefs_).then(function () {
        var opts = {
          load_settings: true
        };
  ...
  this.onEnableDuckingChange = function () {
    var disable = $("#enable_ducking")[0].checked === false;
    ...
    var form = $("#manualduckinglist")[0];
    [].slice.call(form.elements).forEach(function (elem) {
      elem.disabled = disable;
    });

    var $divs = $("div, label", $(".music_ducking_section"));
    [].slice.call($divs).forEach(function (elem) {
      if (elem.id === "enable_ducking_div" || elem.id === "enable_ducking_label") return; // don't grey out the enable ducking div

      if (disable) elem.classList.add("disabled");else elem.classList.remove("disabled");
    });

    var rawFiltersButton = $(".music_ducking_section .rawFiltersButton")[0];
    if (disable) rawFiltersButton.classList.add("disabled");else rawFiltersButton.classList.remove("disabled");

    if (!disable) $("#muted_remember_same_domain")[0].checked = true;
    $("#muted_remember_same_domain")[0].disabled = !disable;
  };

  this.onEnableMinTimeBeforeDuckingChange = function () {
    var disable = $("#enable_min_time_before_ducking")[0].checked === false;
    $("#min_time_before_ducking")[0].disabled = disable;
  };
...
},{"./prefs_store":4,"./util":5}],4:[function(require,module,exports){
"use strict";
...
},{"./util":5,"q":1}],5:[function(require,module,exports){
"use strict";
