...
var chromeMisc = require('./background/chrome_misc')(chrome);
var musicControllers = require('./background/music_controllers')(chrome);
var prefsStore = require('./prefs_store')(chrome);

var URL_OPTIONS = chrome.runtime.getURL('build/html/options.html');
var URL_WEBSTORE = 'https://chrome.google.com/webstore/detail/acofndgbcimipbpeoplfjcapdbebbmca';
var URL_SUPPORT = 'http://www.github.com/jaredsohn/mutetab/issues';

// UI constants; not used right now (requires ui shown via keyboard shortcuts)
var PADDING_TOP = 50;
var PADDING_BOTTOM = 50;
var EXTENSION_UI_WIDTH = 300;
...
var URL_CHANGE_WAIT = 10;
var DUCKED_TIMEOUT_EXTRA_WAIT = 60; // wait this additional length before clearing something that is ducked.  Otherwise it gets unducked too quickly.

var hideDucking_ = false;
var injectingEnabled_ = false;

// globals
var prefs_ = {}; // User preferences
var tabState_ = {}; // Current state.
var updateContextMenusTimeout_ = null;
var refreshUiTimeout_ = null;
var isFirstTime_ = true;
var updateContextMenusBusy_ = false;
var browserActionTitle_ = 'MuteTab';
var browserActionUnduckMessage_ = '';
var browserActionMode_ = '';
var checkMaybeAudibleCount_ = 0;
var privacyModeToggleInProgress_ = false;
var prevDuckingTabState_ = null;

// Turn on/off logging; can be set via console
var loggingEnabled_ = void 0;
var logTypes_ = ['duckingReasoning', 'injected', 'events', 'music', 'ui'];
var logTypeEnabled_ = void 0; // a dict that indicates if each logtype is enabled or not

// Music ducking state
var musicDuckingIntervalId_ = null;
var duckingCountDown_ = null;
var unduckingOrder_ = []; // tabs that are ducked, in order.  We'll skip over entries if not audible, though.
var unduckedTabId_ = -1;
var unduckedShortTimeTabId_ = -1;
var updateDuckingCount_ = 0;

// Field names that start with '_' are should only be used for debugging purposes
var fields_ = ['mutedCached', 'audibleCached', 'domainCached', 'hasCustomMusicController', 'nonPrivateMuteStatus', 'nonPrivatePlayStatus', 'musicStateData', 'musicStateForFrame', 'tabCaptured', 'mutingError', 'lastAudibleStart', 'lastAudibleEnd', 'lastAudibleEndBackup', 'lastUnmuted', 'lastPlayed', 'lastPaused', 'lastAudibleAndUnmuted', 'playInProgress', 'pauseInProgress', 'checkMaybeAudible', 'urlChanged', 'playInProgressExpired', 'pauseInProgressExpired', 'playInProgressReason', 'pauseInProgressReason', 'mutedReason', 'playPauseReason', 'url', 'longestPrevDuration', 'ducked', '_audibleTooShortCached', '_inaudibleTooLongCached', '_prevAudibleDuration', '_timeSinceAudibleBecameFalse'];

var monitoredFields_ = ['lastAudibleStart', 'audibleCached', 'lastPlayed', 'lastPaused', 'urlChanged', 'musicStateData', 'nonPrivateMuteStatus'];

///////////////////////////////////////////////////////////////////////////////////////////////////
// Tab State
///////////////////////////////////////////////////////////////////////////////////////////////////

var getDefaultState = function getDefaultState(fieldName) {
  switch (fieldName) {
    case 'mutedCached':
    case 'audibleCached':
    case 'ducked':
...
var getState = function getState(tabId, fieldName) {
  if (fields_.indexOf(fieldName) === -1) {
    console.error('Invalid field: ' + fieldName);
    return null;
  }

  return !tabState_.hasOwnProperty(tabId) || !tabState_[tabId].hasOwnProperty(fieldName) ? getDefaultState(fieldName) : tabState_[tabId][fieldName];
};

var setState = function setState(tabId, fieldName, val, reason) {
  if (fields_.indexOf(fieldName) === -1) {
    console.error('Invalid field: ' + fieldName);
    return;
  }

  var defaultFieldName = getDefaultState(fieldName);

  if (!tabState_.hasOwnProperty(tabId)) tabState_[tabId] = {};
  ...
  if (monitoredFields_.indexOf(fieldName) >= 0) console.log(tabId, 'monitored', fieldName, val, typeof reason !== 'undefined' ? reason : '');
};

var getAllTabIds = function getAllTabIds() {
  return Object.keys(tabState_).map(function (tabId) {
...
var getStateForAllTabs = function getStateForAllTabs(fieldName) {
  var obj = {};
...
var getFullState = function getFullState(tabId) {
  return tabState_.hasOwnProperty(tabId) ? tabState_[tabId] : {};
};

var getFullStateForAllTabs = function getFullStateForAllTabs() {
  return tabState_;
};

var clearState = function clearState(tabId) {
  console.log(tabId, 'clearing state');
  delete tabState_[tabId];
};

var clearStateFieldForAllTabs = function clearStateFieldForAllTabs(fieldName) {
  if (fields_.indexOf(fieldName) === -1) {
    console.error('Invalid field: ' + fieldName);
    return;
  }
...
var muteEverything = function muteEverything() {
  return windowManager.getTabs().done(function (historyData) {
    console.log('muteeverything!');

    var mutePromises = historyData.map(function (tabInfo) {
      return updateMuted(tabInfo.id, true, { saveNonPrivate: false }, 'Muted due to privacy mode.');
    });
    var pausePromises = historyData.map(function (tabInfo) {
      return pauseMusic(tabInfo.id, 'Paused due to privacy mode.');
    });

    return Q.allSettled(mutePromises).then(Q.allSettled(pausePromises));
  });
};

// Excludes music list
var muteAll = function muteAll(excludeWhiteListedTabs) {
  return windowManager.getTabs().then(function (tabs) {
    if (typeof excludeWhiteListedTabs === 'undefined') excludeWhiteListedTabs = true;

    var filtered = tabs.filter(function (tabInfo) {
      var domain = getDomain(tabInfo.url);
      return (!excludeWhiteListedTabs || !prefsStore.domainInList(domain, prefs_.whitelist)) && !prefsStore.domainInList(domain, prefs_.musiclist);
    });
    var mutePromises = filtered.map(function (tabInfo) {
      return updateMuted(tabInfo.id, true, {}, 'Muted by \'Mute all tabs\'.');
    });
    var pausePromises = filtered.map(function (tabInfo) {
...
var unmuteAll = function unmuteAll() {
  return windowManager.getTabs().then(function (tabs) {
    var mutePromises = tabs.map(function (tabInfo) {
...
var makeCurrentTabOnlyUnmuted = function makeCurrentTabOnlyUnmuted() {
  muteBackground();
  Q.when(windowManager.getCurrentTab()).then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
...
var muteBackground = function muteBackground() {
  return Q.all([windowManager.getTabs(), windowManager.getCurrentTab()]).spread(function (tabs, currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
    var filtered = tabs.filter(function (tabInfo) {
    ...
    var mutePromises = filtered.map(function (tabInfo) {
      return updateMuted(tabInfo.id, true, {}, 'Muted by \'Mute background tabs\'.');
    });
    var pausePromises = filtered.map(function (tabInfo) {
...
var toggleCurrentMuted = function toggleCurrentMuted() {
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
...
var setCurrentMuted = function setCurrentMuted(mute) {
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
    var reason = mute ? 'Muted by user via MuteTab context menu.' : 'Unmuted by user via MuteTab context menu';
...
var muteAudible = function muteAudible() {
  return windowManager.getTabs().then(function (tabs) {
    var filtered = tabs.filter(function (tabInfo) {
      return !getState(tabInfo.id, 'mutedCached') && getState(tabInfo.id, 'audibleCached');
    });
    var promises = filtered.map(function (tabInfo) {
...
var onCreated = function onCreated(tab) {
  if (prefs_.disableAutomuting) return;
  chromeMisc.ensureMutedInfo(tab);

  var muteInfo = null;

  try {
    if (prefs_.muteAllTabs) muteInfo = { should: true, save: true, reason: 'Muted by default' };else if (tab.incognito && prefs_.muteNewIncognito) muteInfo = { should: true, save: true, reason: 'Incognito muted by default' };else muteInfo = { should: false, save: true, reason: '' };
...
var onReplaced = function onReplaced(addedTabId, removedTabId) {
  console.log("ONREPLACED!");
  if (logTypeEnabled_.events) console.log('onReplaced', addedTabId, removedTabId);
  if (windowManager.getLastTabIdSync() === removedTabId) windowManager.setLastTabId(addedTabId); // Update this so that we don't mute the wrong tab in mute background
  ...
  var duckedTabIndex = unduckingOrder_.indexOf(removedTabId);
  if (duckedTabIndex !== -1) unduckingOrder_[duckedTabIndex] = addedTabId;

  windowManager.getTabInfo(addedTabId).then(function (tabInfo) {
    updateStateForUrlChange(tabInfo);
  }).done();
};

var onUpdated = function onUpdated(tabId, changeInfo, tab) {
  if (tabId === windowManager.extensionWindowIdSync) return;
  chromeMisc.ensureMutedInfo(tab);
  chromeMisc.ensureMutedInfo(changeInfo);

  var urlChanged = false;
  var oldUrl = getState(tab.id, 'url') || null;
  if (oldUrl !== tab.url) {
    urlChanged = true;
  }
  ...
  if (changeInfo.hasOwnProperty('status') && changeInfo.status === 'loading') {
    if (urlChanged) {
      if (logTypeEnabled_.events) console.log('onUpdated (status=loading)', tabId, tab.url, changeInfo, tab);
      updateStateForUrlChange(tab).done();
    } else {
      if (logTypeEnabled_.events) console.log('(status=loading) but url did not change.', tabId, tab.url, changeInfo, tab);
    }
  } else if (urlChanged) {
    if (logTypeEnabled_.events) console.log('onUpdated urlchanged', tabId, tab.url, changeInfo, tab);
    updateStateForUrlChange(tab).done();
  }

  if (changeInfo.hasOwnProperty('audible')) {
    if (logTypeEnabled_.events) console.log(tabId, 'onUpdated - audible', changeInfo.audible === true, getState(tabId, 'mutedCached'), getState(tabId, 'lastAudibleStart').getTime() === 0);
    if (changeInfo.audible === true && getState(tabId, 'mutedCached') && getState(tabId, 'lastAudibleStart').getTime() === 0) {
      localStorage.noisesPrevented = parseInt(localStorage.noisesPrevented || 0, 10) + 1;
    }
    if (logTypeEnabled_.events) console.log('onUpdated', tabId, tab.url, changeInfo, tab);
  ...
  if (changeInfo.hasOwnProperty('mutedInfo')) {
    ...
    if (changeInfo.mutedInfo.extensionId !== chrome.runtime.id) {
      if (logTypeEnabled_.events) console.log('onUpdated', tabId, tab.url, changeInfo, tab);
      var mutedCauseText = changeInfo.mutedInfo.reason;
      if (mutedCauseText === 'extension') mutedCauseText = 'another extension. (id=' + changeInfo.mutedInfo.extensionId + ')';else if (mutedCauseText === 'user') mutedCauseText = 'user through Chrome.';else mutedCauseText += '.';
...
var onActivated = function onActivated(activeInfo) {
  windowManager.getCurrentTab().then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return;
    }
    ...
    if (activeInfo.windowId === windowManager.getExtensionWindowIdSync()) return;

    var prevTabId = windowManager.getLastTabIdSync();
    ...
    windowManager.getTabInfo(activeInfo.tabId).then(function (tabInfo) {
      ...
      if (!prefs_.disableAutomuting && prefs_.muteBackgroundTabs) {
        var isMusic = prefsStore.domainInList(getState(prevTabId, 'domainCached'), prefs_.musiclist);
        ...
        if (!isMusic) updateMuted(prevTabId, true, {}, 'Background muted by default.').done();
      }

      if (getState(activeInfo.tabId, 'ducked')) {
        var timeToCheck = Math.min(getCountdownForUnducking(activeInfo.tabId, false), prefs_.minTimeBeforeUnducking);
...
var onRemoved = function onRemoved(tabId) {
  //if (logTypeEnabled_.events)
  // console.log("OnRemoved", tabId);

  var domainText = '';
  var domainCached = getState(tabId, 'domainCached');
  if (domainCached !== null) domainText = ' (' + domainCached + ')';
  clearState(tabId);
  removeFromArray(unduckingOrder_, tabId);
  if (prefs_.enableDucking) updateDucking('removed tab: ' + tabId + domainText).done(); // don't worry about race conditions with this
  if (logTypeEnabled_.events) console.log(tabId, 'tab closed');
  refreshUi();
};
...
var onCommand = function onCommand(command) {
  onCommandAsPromise(command).done();
};
var onCommandAsPromise = function onCommandAsPromise(command) {
  if (logTypeEnabled_.events) console.log('onCommand:', command);

  var promise = Q.when(null); // our default promise, used when calling sync code
  switch (command) {
    case 'make_current_tab_only_unmuted':
      return makeCurrentTabOnlyUnmuted();
    case 'show_tabs_window':
      return showTabsWindow();
    case 'mute_all':
      return muteAll(false);
    case 'unmute_all':
      return unmuteAll();
    case 'mute_background':
      return muteBackground();
    case 'mute_current':
      return setCurrentMuted(true);
    case 'unmute_current':
      return setCurrentMuted(false);
    case 'mute_audible':
      return muteAudible();
    case 'toggle_current_muted':
      return toggleCurrentMuted();
    case 'blacklist_remove':
      return updateListForCurrentTab('neither');
    case 'blacklist_add':
      return updateListForCurrentTab('black');
    case 'whitelist_remove':
      return updateListForCurrentTab('neither');
    case 'whitelist_add':
      return updateListForCurrentTab('white');
    case 'musiclist_remove':
      return updateListForCurrentTab('notmusic');
    case 'musiclist_add':
      return updateListForCurrentTab('music');
    case 'manualduckinglist_remove':
      return updateListForCurrentTab('notmanualduckinglist');
    case 'manualduckinglist_add':
      return updateListForCurrentTab('manualducking');
    case 'change_privacy_mode':
      return togglePrivacyMode();
    case 'change_disable_automuting':
      prefs_.disableAutomuting = !prefs_.disableAutomuting;if (!prefs_.disableAutomuting) {
        updateDucking('automuting reenabled');setFavicon();
      } else {
        setFavicon();
      }return prefsStore.save(prefs_);
    case 'show_options':
      return windowManager.openUrl(URL_OPTIONS);
    case 'show_webstore':
      return windowManager.openUrl(URL_WEBSTORE);
    case 'show_support':
      return windowManager.openUrl(URL_SUPPORT);
    case 'load_settings':
      return loadSettings();
    case 'pause_current':
      return pauseCurrent();
    case 'play_current':
      return playCurrent();
    case 'mute_unducked':
      return updateMuted(unduckedTabId_, true, {}, 'Muted by user as \'unducked tab\' via MuteTab context menu.');
    case 'pause_unducked':
      return pauseMusic(unduckedTabId_, 'Paused by user as \'unducked tab\' via MuteTab context menu.');
    case 'close_unducked':
      return windowManager.closeTab(unduckedTabId_);
    case 'show_unducked':
      return windowManager.switchToTab(unduckedTabId_);
    case 'mute_unducked':
      return muteOrPauseUnducked();

    default:
      console.warn('Unsupported command requested: ' + command);
  }
...
var onMessage = function onMessage(request, sender, respond) {
  var keys = Object.keys(request);
  var showMessageInConsole = true;
  var doNotLogKeys = ['created', 'action', 'send_tab_data'];
  doNotLogKeys.forEach(function (doNotLogKey) {
    if (request.hasOwnProperty(doNotLogKey)) showMessageInConsole = false;
  });
  if (showMessageInConsole && logTypeEnabled_.events) console.log('onMessage: ' + keys, request, sender);
  ...
  keys.forEach(function (key) {
    switch (key) {
      ...
      case 'toggle_current_muted':
      case 'mute_audible':
        onCommandAsPromise(key).done(function (data) {
          respond(data);
        });break; // If no parameters, then use onCommand code.

      case 'change_enableDucking':
        prefs_.enableDucking = request[key];return prefsStore.save(prefs_).then(updateDucking('change_enableDucking')).done(respond);
      case 'switch_to_tab':
        windowManager.switchToTab(request[key]).done(respond);break;
      case 'switch_to_tabs':
        windowManager.switchToTabs(request[key]).done(respond);break;
      case 'close_tab':
        windowManager.closeTab(request[key]).done(respond);break;
      case 'create_tab':
        windowManager.createTab({ url: request[key] }).done(respond);break;
      case 'create_tabs':
        windowManager.createTabs(request[key]).then(windowManager.switchToTab(sender.tab.id)).done(respond);break;
      case 'change_url':
        windowManager.changeUrl(request[key].tabId, request[key].url).done(respond);break;
      case 'change_all_urls':
        windowManager.changeAllUrls().done(respond);break;
      case 'set_muted':
        updateMuted(request[key].tabId, request[key].muted, {}, (request[key].muted ? 'Muted' : 'Unmuted') + ' by user via MuteTab.').done(respond);break;
      case 'set_audible':
        updateAudible(request[key].tabId, request[key].audible, 'tests');respond({});return false;
      case 'send_tab_data':
        sendTabData(sender.hasOwnProperty('tab') ? sender.tab.id : -1).done(respond);break;
      case 'update_listtype':
        prefsStore.updateListAndSave(prefs_, request[key].listType, request[key].domain).then(updateContextMenus()).done(respond);break;
      case 'change_show_other_tabs':
        prefs_.showOtherTabs = request[key];return prefsStore.save(prefs_).done(respond);
      case 'play_music':
        /*ignore jslint start*/playMusic(request[key], 'Played by user via MuteTab.').then(updateMuted(request[key], false, { skipPlay: true }, 'Unmuted by user when playing via MuteTab.')).done(respond);break; /*ignore jslint end*/ // Note: cannot depend on this alerting when operation done; linting broken since catch here is from q
      case 'pause_music':
        /*ignore jslint start*/pauseMusic(request[key], 'Paused by user via MuteTab.').done(respond);break; /*ignore jslint end*/ // Note: same as for play_music
      case 'setup_test_scenario':
        setupTestScenario(sender.hasOwnProperty('tab') ? sender.tab.id : -1, sender.hasOwnProperty('tab') ? sender.tab.windowId : -1, request[key]).done(respond);break;
      case 'set_properties_multiple':
        setPropertiesMultiple(request[key].tabIds, request[key].properties).done(respond);break;
      case 'get_prefs':
        respond(prefs_);return false;
      case 'set_prefs':
        prefs_ = request[key];console.log('prefs set (in memory) to', prefs_);afterLoadSettings().then(setFavicon()).done(respond);break;
      // These messages come from music controllers (which originated with StreamKeys).
      case 'stateData':
        updateMusicStateData(sender.hasOwnProperty('tab') ? sender.tab.id : -1, request[key]).done(function () {});return false;
      case 'created':
      case 'action':
        return false; // ignore these messages
      default:
        console.warn('Unsupported onMessage key: ' + key, request[key]);break;
    }
    return false;
  });
...
var onContextMenuClicked = function onContextMenuClicked(info) {
...
var updateMuted = function updateMuted(tabId, shouldMute, options, reason) {
  ...
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    ...
    options = options || {};
    var skipPlay = options.skipPlay || false;

    var maybeMutePromise = chromeMisc.setMuted(tabId, shouldMute);
    var maybePlayPromise = !skipPlay && shouldMute === false && getState(tabId, 'ducked') ? playMusic(tabId, 'Played because unmuted.') : Q.when(null);

    return Q.allSettled([maybeMutePromise, maybePlayPromise]).then(function (output) {

      if (output[0].value === undefined) return Q.when(null);
      ...
      var saveNonPrivate = options.hasOwnProperty('saveNonPrivate') ? options.saveNonPrivate : true;
      var shouldUpdateDucking = options.hasOwnProperty('shouldUpdateDucking') ? options.shouldUpdateDucking : true;
      var recordLastUnmuted = options.hasOwnProperty('recordLastUnmuted') ? options.recordLastUnmuted : true;

      var muteChanged = getState(tabId, 'mutedCached') !== shouldMute;
      ...
      if (muteChanged) setState(tabId, 'mutedReason', reason);

      if (saveNonPrivate) setState(tabId, 'nonPrivateMuteStatus', shouldMute);

      refreshUi();

      if (currentTabInfo.id === tabId || unduckedTabId_ === tabId) updateContextMenus();
...
var updateAudible = function updateAudible(tabId, audible, reason) {
  var timeToCheck = void 0;
  var now = new Date();

  if (logTypeEnabled_.events) console.log(tabId, 'updateAudible', audible, reason);
  ...
  if (!audible) {
    ...
    if (getCountdownForUnducking(tabId, true) > 0) {
      ...
      if (getState(tabId, 'lastAudibleStart') <= getState(tabId, 'lastAudibleEnd')) setState(tabId, 'lastAudibleStart', now);
    } else {
      setState(tabId, 'lastAudibleStart', now);
    }
    setState(tabId, 'audibleCached', audible);

    if (!getState(tabId, 'mutedCached') && maybeAudible(unduckedTabId_) && tabId !== windowManager.getLastTabIdSync()) {
  ...
  timeToCheck = audible ? prefs_.minTimeBeforeDucking : Math.min(getCountdownForUnducking(tabId, false), prefs_.minTimeBeforeUnducking);
...
var updateListForCurrentTab = function updateListForCurrentTab(listType) {
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
    var domainToUpdate = getDomain(currentTabInfo.url);
    if (listType == 'white' || listType == 'black') {
      var blackOrWhiteListDomain = prefsStore.getDomainRuleForDomainInList(domainToUpdate, prefs_[listType + 'list']);
      if (blackOrWhiteListDomain !== null) domainToUpdate = blackOrWhiteListDomain;
    }
    return prefsStore.updateListAndSave(prefs_, listType, domainToUpdate).then(updateContextMenus()).done();
  });
};
...
var togglePrivacyMode = function togglePrivacyMode() {
  if (privacyModeToggleInProgress_) return Q.when(null);
...
var setPrivacyModeOn = function setPrivacyModeOn() {
  return Q.when(null, function () {
    ...
    Object.keys(getStateForAllTabs('musicStateData')).forEach(function (tabId) {
      if (getMusicState(tabId, 'isPlaying')) setState(tabId, 'nonPrivatePlayStatus', true);
    });
    ...
    return prefsStore.save(prefs_).then(Q.allSettled(mutePromises)).then(Q.allSettled(playPromises)).then(updateDucking('privacy mode enabled')) // shouldn't do anything here
    .then(updateContextMenus()).then(setFavicon()).then(function () {
...
var setPrivacyModeOff = function setPrivacyModeOff() {
  return Q.when(null, function () {
    ...
    return windowManager.getTabs().then(function (tabs) {
      var maybeAudibleUnmutedFound = false;
      ...
      if (maybeAudibleUnmutedFound) needToDuckIds.push(unduckedTabId_);
      ...
      return Q.allSettled(mutePromises).then(Q.allSettled(playPromises)).then(Q.allSettled(duckTabIds(needToDuckIds))).then(function () {
        prefs_.privacyMode = false;
        return prefsStore.save(prefs_).then(updateDucking('privacy mode disabled')).then(updateContextMenus()).then(setFavicon()).then(function () {
          privacyModeToggleInProgress_ = false;
          sendTabData(windowManager.getExtensionTabIdSync()).done();
...
var updateStateForUrlChange = function updateStateForUrlChange(tab) {
  chromeMisc.ensureMutedInfo(tab);
  console.log(tab.id, 'updateStateForUrlChange', tab.url);
  var promise = Q.when(null);
  if (prefs_.muteBackgroundTabs) promise = windowManager.getLastTabId();

  return promise.then(function (lastTabId) {
    // Note that this async can cause tab muting do vary in order (which can break tests)
    var muteInfo = null;
    var domain = getDomain(tab.url);
    var oldDomain = getState(tab.id, 'domainCached');
    var oldDucked = getState(tab.id, 'ducked');
    var oldAudible = getState(tab.id, 'audibleCached');
    var oldAudibleStart = getState(tab.id, 'lastAudibleStart');

    // console.log("domain: ", domain, getState(tab.id, "domainCached"));

    if (domain !== null) {
      var wasMuted = getState(tab.id, 'mutedCached');
      ...
      if (prefs_.mutedRememberSameDomain && domain === oldDomain && !prefs_.disableAutomuting) {
        ...
        var isWhiteList = prefsStore.domainInList(domain, prefs_.whitelist);
        var isBlackList = prefsStore.domainInList(domain, prefs_.blacklist);
        var isMusic = prefsStore.domainInList(domain, prefs_.musiclist);
        ...
        if (prefs_.disableAutomuting || getState(tab.id, 'ducked')) {
          ...
          if (isMusic) muteInfo = { should: false, reason: 'Not muted by default since on music list.' };else if (isWhiteList) muteInfo = { should: false, reason: 'Not muted by default since on whitelist.' };else muteInfo = { should: true, reason: 'Muted by default.' };
        } else if (prefs_.muteBackgroundTabs) {
          // mute background
          if (tab.id === lastTabId) {
            if (isBlackList) muteInfo = { should: true, reason: 'Muted because on blacklist.' };else muteInfo = { should: false, reason: 'Not muted by default since foreground tab.' };
          } else {
            if (isMusic) muteInfo = { should: false, reason: 'Not muted by default since on music list.' };else muteInfo = { should: true, reason: 'Background muted by default.' };
          }
        } else if (prefs_.unmuteAllTabs) {
          // unmuted by default
          if (isBlackList) muteInfo = { should: true, reason: 'Muted because on blacklist.' };else muteInfo = { should: false, reason: 'Unmuted by default.' };
        }
      }
    ...
    var maybeMutePromise = muteInfo ? updateMuted(tab.id, muteInfo.should, {}, muteInfo.reason) : Q.when(null);
    maybeMutePromise.then(function () {
    ...
    }).then(injectMusicApi(tab.id, tab.url, false)).then(function () {
...
var updateContextMenus = function updateContextMenus() {
  ...
  updateContextMenusTimeout_ = setTimeout(function () {
    ...
    return windowManager.getCurrentTab().then(function (currentTabInfo) {
      ...
      return chromeMisc.contextMenusRemoveAll().then(function () {
        try {
          var domain = getDomain(currentTabInfo.url);
          var listType = prefs_.muteAllTabs || currentTabInfo.incognito && prefs_.muteNewIncognito ? 'white' : 'black';
          var inList = prefsStore.domainInList(domain, prefs_[listType + 'list']);
          var blackOrWhiteListDomain = prefsStore.getDomainRuleForDomainInList(domain, prefs_[listType + 'list']);
          var inMusicList = prefsStore.domainInList(domain, prefs_.musiclist);
          var inManualDuckingList = prefsStore.domainInList(domain, prefs_.manualduckinglist);
          var unduckedTabContextId = void 0;

          if (blackOrWhiteListDomain === null) blackOrWhiteListDomain = domain;

          var currentTabContextId = chrome.contextMenus.create({
            'title': 'Current tab',
            'contexts': ['page']
          });

          var backgroundTabsContextId = chrome.contextMenus.create({
            'title': 'Background tabs',
            'contexts': ['page']
          });

          var allTabsContextId = chrome.contextMenus.create({
            'title': 'All tabs',
            'contexts': ['page']
          });
          ...
          var isPlaying = getMusicState(currentTabInfo.id, 'isPlaying');
          if (isPlaying !== '') {
            chrome.contextMenus.create({
          ...
          var blackWhiteListCommand = listType === 'black' ? inList ? 'blacklist_remove' : 'blacklist_add' : inList ? 'whitelist_remove' : 'whitelist_add';
...
var showTabsWindow = function showTabsWindow() {
  return Q.all([windowManager.getLastFocusedWindow(), windowManager.getExtensionWindowId()]).spread(function (currentWindow, extensionWindowId) {
    ...
    var left = currentWindow.left + Math.round((currentWindow.width - EXTENSION_UI_WIDTH) / 2);
    var top = currentWindow.top + PADDING_TOP;
    var height = Math.max(currentWindow.height - PADDING_TOP - PADDING_BOTTOM, 600);
    var width = EXTENSION_UI_WIDTH;

    return windowManager.showExtensionUi(width, height, left, top);
  });
};

var updateTabInfoWithMetaData = function updateTabInfoWithMetaData(tabInfo) {
  try {
    var domain = getDomain(tabInfo.url);
...
var sendTabData = function sendTabData(senderTabId) {
  try {
    return windowManager.getTabs().then(function (tabs) {

      var listType = prefs_.muteAllTabs || prefs_.muteBackgroundTabs ? 'white' : 'black';
      var tabIdDict = {};
      var currentTabs = [];
      var mostRecentlyAudibleTabs = [];
      var audibleTabs = [];
      var recentlyAudibleTabs = [];
      var duckedTabs = [];
      var musicTabs = [];
      var otherTabs = [];
      ...
      unduckingOrder_.forEach(function (tabId) {
        if (!getState(tabId, 'ducked')) return;
        var tabInfo = tabIdDict[tabId];
        ...
        tabs.forEach(function (tabInfo) {
          try {
            ...
            if (tabInfo.id === windowManager.getLastTabIdSync()) {
              var currentTabInfo = JSON.parse(JSON.stringify(tabInfo));
            ...
            if ((tabInfo.audible || tabInfo.isPlaying) && !tabInfo.mutedInfo.muted && getState(tabInfo.id, 'lastAudibleStart').getTime()) {
      ...
      var tabLists = [currentTabs, audibleTabs, mostRecentlyAudibleTabs, duckedTabs, recentlyAudibleTabs, musicTabs, otherTabs];
      ...
      if (logTypeEnabled_.ui) console.log('sendTabData data', newData);
      return newData;
    });
  } catch (ex) {
    console.error(ex);
...
var refreshUi = function refreshUi() {
  ...
  refreshUiTimeout_ = setTimeout(function () {
    ...
    views.forEach(function (view) {
      if (view.location.href === chrome.extension.getURL('build/html/popup.html')) popupFound = true;
    });
    if (popupFound) {
      // console.log("refresh ui!");
      if (typeof reactUi_ !== 'undefined') reactUi_.refreshTabs();
    }
...
var refreshUiCountDown = function refreshUiCountDown() {
  var displayedCountDownVal = duckingCountDown_;
  displayedCountDownVal = Math.ceil(displayedCountDownVal);
  var alreadyAudibleOrPlaying = unduckedTabId_ !== -1 && (getAudibleOrPlaying(unduckedTabId_) || false);
  ...
  if (prefs_.minTimeBeforeUnducking >= prefs_.audioNotifierDelay + 1 && !alreadyAudibleOrPlaying && getFirstDuckedTabId() && displayedCountDownVal > 0 && displayedCountDownVal !== Number.MAX_SAFE_INTEGER) {
...
var getMutingBehaviorString = function getMutingBehaviorString() {
  var str = '';
  if (prefs_.disableAutomuting) {
    ...
    if (prefs_.muteAllTabs) str += ', mutes tabs by default';else if (prefs_.muteBackgroundTabs) str += ', mutes background tabs by default';
    if (prefs_.muteNewIncognito) str += ', mutes incognito tabs by default';
    if (prefs_.mutedRememberSameDomain) str += ', remembers muted for same domain';
    if (prefs_.enableDucking) str += ', ducks music';
    ...
    var inManualDuckingList = false;
    if (unduckedTabId_ !== -1 && duckingEffectivelyEnabled()) inManualDuckingList = prefsStore.domainInList(getState(unduckedTabId_, 'domainCached'), prefs_.manualduckinglist);
    if (inManualDuckingList && getState(unduckedTabId_, 'lastAudibleStart').getTime()) str += '\nManual intervention required to return to music in another tab.';
  }
  return str;
};
...
var setFavicon = function setFavicon() {
  return Q.fcall(function () {
    var faviconFileName = 'build/img/favicon.png';
...
var clearPlayAndPauseInProgressIfExpired = function clearPlayAndPauseInProgressIfExpired(tabId) {
  var playInProgressTimestamp = getState(tabId, 'playInProgress');
  ...
  var pauseInProgressTimestamp = getState(tabId, 'pauseInProgress');
...
var playMusic = function playMusic(tabId, reason) {
  if (prefs_.disablePlayPause) return Q.when(null);

  return Q.fcall(function () {
    if (isPlayingOrPlayInProgress(tabId)) return Q.when(null);

    if (logTypeEnabled_.music) console.log(tabId, 'playMusic', reason, getMusicState(tabId, 'isPlaying'));
...
var pauseMusic = function pauseMusic(tabId, reason) {
  if (prefs_.disablePlayPause) return Q.when(null);

  return Q.fcall(function () {
    if (!isPausedOrPauseInProgress(tabId)) {
      if (logTypeEnabled_.music) console.log(tabId, 'pauseMusic', reason);
...
var pauseCurrent = function pauseCurrent() {
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    if (!currentTabInfo) {
      return Q.when(null);
    }
...
var playCurrent = function playCurrent() {
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    ...
    return updateMuted(currentTabInfo.id, false, {}, 'Unmuted by user when playing via MuteTab context menu.').then(playMusic(currentTabInfo.id, 'Played by user via MuteTab context menu.'));
  });
};

var muteOrPauseUnducked = function muteOrPauseUnducked() {
  if (unduckedTabId_ !== -1) {
    return getMusicState(unduckedTabId_, 'isPlaying') ? pauseMusic(unduckedTabId_, 'Paused by user as \'unducked tab\' via keyboard shortcut.') : updateMuted(unduckedTabId_, true, {}, 'Muted by user as \'unducked tab\' via keyboard shortcut.');
  }
  return Q.when(null);
};

var injectMusicApi = function injectMusicApi(tabId, url, injectDefault) {
  ...
  if (url.startsWith('chrome://')) return Q.when(false);
  if (url.startsWith('chrome-devtools://')) return Q.when(false);
  if (url.startsWith('chrome-extension://')) return Q.when(false);
  if (url.startsWith('https://chrome.google.com/webstore')) return Q.when(false);
  if (url.startsWith('data:')) return Q.when(false);

  if (logTypeEnabled_.injected) console.log(tabId, 'injectMusicApi', url, injectDefault);

  return Q.when(null).then(function () {
    var promises = [Q.when(null)];

    var disableDefaultController = false;
    var allFrames = false;
    var controllerFileName = musicControllers.getController(url);

    if (controllerFileName !== null) setState(tabId, 'hasCustomMusicController', true);

    if (controllerFileName !== null) {
      //disableDefaultController = (controllerFileName === "YoutubeController.js");
      disableDefaultController = true;
      if (logTypeEnabled_.injected) console.log(tabId, 'controllerFileName', controllerFileName);
    ...
    return Q.allSettled(promises).then(function () {
      ...
      return disableDefaultController ? chromeMisc.tabsSendMessage(parseInt(tabId, 10), { 'action': 'disable' }, {}) : Q.when(true);
    });
  }).catch(function (err) {
    console.error(err);
  });
...
var computeStateDataAcrossFrames = function computeStateDataAcrossFrames(tabId) {
  // console.log(tabId, "computeStateDataAcrossFrames", musicStateForFrame_[tabId]);
  var playCount = 0;
  var pauseCount = 0;
  var savePausedCount = 0;
  var noContentCount = 0;
  var siteName = null;
  var musicStateForFrame = getState(tabId, 'musicStateForFrame');
  var keys = Object.keys(musicStateForFrame);
  keys.forEach(function (key) {
    ...
    if (siteName === null) siteName = getState(tabId, 'musicStateForFrame')[key].siteName;
  });
  if (logTypeEnabled_.injected) console.log(tabId, 'computeAcrossFrames', playCount, pauseCount, savePausedCount, noContentCount);

  var musicStateData = {};
  if (playCount > 0) musicStateData.isPlaying = true;else if (savePausedCount > 0 || pauseCount === 1) // only allow clicking play if there is just one source
    musicStateData.isPlaying = false;else musicStateData.isPlaying = null;
...
var okayToUpdateAudibleOrPlaying = function okayToUpdateAudibleOrPlaying(tabId) {
  return !prefs_.enableDucking || getNeverAudibleForUrl(tabId) || !prefsStore.domainInList(getState(tabId, 'domainCached'), prefs_.manualduckinglist);
};

var updateMusicStateData = function updateMusicStateData(tabId, musicStateData) {
  if (tabId === windowManager.getExtensionTabIdSync()) return Q.when(null);

  if (prefs_.disablePlayPause) return Q.when(null);

  // Ignore HTML5 player state data for sites that have a custom player
  if (getState(tabId, 'hasCustomMusicController') === true && (musicStateData.fromDefaultController || null)) return Q.when(null);
  ...
  return windowManager.getCurrentTab().then(function (currentTabInfo) {
    ...
    if (musicStateData.hasOwnProperty('frameId')) {
      var musicStateForFrames = getState(tabId, 'musicStateForFrame');
    ...
    var storedMusicStateData = getState(tabId, 'musicStateData');

    if (!storedMusicStateData) {
      // Set if first info for tab
      firstInfo = true;
      setState(tabId, 'musicStateData', musicStateData);
    } else if (storedMusicStateData.isPlaying !== musicStateData.isPlaying || storedMusicStateData.artist !== musicStateData.artist || storedMusicStateData.song !== musicStateData.song || storedMusicStateData.autoplayCountdown !== musicStateData.autoplayCountdown) {
    ...
    if (firstInfo || storedMusicStateData.isPlaying !== musicStateData.isPlaying) {
      ...
      if (prefs_.enableDucking) {
        var timeToCheck = musicStateData.isPlaying ? prefs_.minTimeBeforeDucking : prefs_.minTimeBeforeUnducking;
        addMaybeAudibleCheck(tabId, timeToCheck, true);
      }
      refreshUi();
      if (logTypeEnabled_.music) console.log(tabId, 'updateMusicStateData done', new Date(), musicStateData);
    }

    if (currentTabInfo.id === tabId || tabId === unduckedTabId_) updateContextMenus();

    return Q.when(null);
  });
};

var getMusicState = function getMusicState(tabId, field) {
  var musicStateData = getState(tabId, 'musicStateData');
  if (musicStateData === null || !musicStateData.hasOwnProperty(field) || musicStateData[field] === null) {
    return '';
  }
...
var moveToFrontOfUnduckingOrder = function moveToFrontOfUnduckingOrder(tabId) {
  console.log(tabId, "moveToFrontOfUnduckingOrder");
  var index = unduckingOrder_.indexOf(tabId);
  if (index >= 0) {
    removeFromArray(unduckingOrder_, tabId);
  }
...
var getLastSilentOrPaused = function getLastSilentOrPaused(tabId) {
  var lastAudibleEnd = getState(tabId, 'lastAudibleEnd');
  var lastPaused = getState(tabId, 'lastPaused');
  var results = lastAudibleEnd > lastPaused ? lastAudibleEnd : lastPaused;
  return results;
};

// Returns when most recently became audible or played (or new Date(0) if neither)
var getLastAudibleOrPlayed = function getLastAudibleOrPlayed(tabId) {
  var lastAudibleStart = getState(tabId, 'lastAudibleStart');
  var lastPlayed = getState(tabId, 'lastPlayed');
  var lastPlayInProgress = getState(tabId, 'playInProgress');
  return new Date(Math.max.apply(null, [lastAudibleStart, lastPlayed, lastPlayInProgress]));
};

var getAudibleOrPlaying = function getAudibleOrPlaying(tabId) {
  var audible = getState(tabId, 'audibleCached') || false;
  // let isPlaying = isPlayingOrPlayInProgress(tabId);
  var isPlaying = getMusicState(tabId, 'isPlaying') === true || new Date().getTime() - getState(tabId, 'playInProgress').getTime() < PLAY_PAUSE_WAIT * 1000;
  return audible || isPlaying;
};

var isPlayingOrPlayInProgress = function isPlayingOrPlayInProgress(tabId) {
...
var isPausedOrPauseInProgress = function isPausedOrPauseInProgress(tabId) {
  clearPlayAndPauseInProgressIfExpired(tabId);

  var result = getMusicState(tabId, 'isPlaying') === false || getState(tabId, 'pauseInProgress').getTime();
  return result;
};

var getNeverAudibleForUrl = function getNeverAudibleForUrl(tabId) {
  ...
  return !getState(tabId, 'audibleCached') && (!getState(tabId, 'lastAudibleStart').getTime() || getState(tabId, 'urlChanged').getTime() > getState(tabId, 'lastAudibleEnd').getTime());
};

// Returns countdown in seconds
var getUrlChangeCountdown = function getUrlChangeCountdown(tabId) {
...
var getCountdownForUnducking = function getCountdownForUnducking(tabId, ignoreAudibleOrPlaying) {
  if (tabId == -1) return 0;
  ...
  if (getState(tabId, 'lastAudibleStart').getTime() && domainIsInList) {
    if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'reason: has been audible and is on manual ducking controls list');
  ...
  var urlChangeCountdown = getUrlChangeCountdown(tabId);
  if (urlChangeCountdown > 0 && tabId === unduckedTabId_) {
    if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'reason: urlchanged; full state is ', getFullState(tabId));
  ...
  if (getNeverAudibleForUrl(tabId)) {
    if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'reason: never audible');
    return 0;
  }
  ...
  if (!ignoreAudibleOrPlaying) {
    ...
    if (tabId === -1 || getAudibleOrPlaying(tabId)) {
      if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'reason: audible or playing');
      return Number.MAX_SAFE_INTEGER;
    }
  }
  ...
  var prevAudibleDuration = getPrevAudibleDuration(tabId);
  ...
  if (prevAudibleDuration > 0) {
    ...
    if (prevAudibleDuration < 0) prevAudibleDuration = 0;
  }

  // Use the user's preference after subtracting away the audio indicator time
  var unduckWait = prefs_.minTimeBeforeUnducking;

  if (getState(tabId, 'hasCustomMusicController') && !getState(tabId, 'audibleCached') && !isPlayingOrPlayInProgress(tabId) && getState(tabId, 'lastPlayed').getTime()) {
  ...
  var duckWait = prefs_.minTimeBeforeDucking - prefs_.audioNotifierDelay;
  var timeSinceAudibleBecameFalse = (now.getTime() - getLastSilentOrPaused(tabId).getTime()) / 1000 + prefs_.audioNotifierDelay;
  setState(tabId, '_timeSinceAudibleBecameFalse', timeSinceAudibleBecameFalse);
  if (prevAudibleDuration < unduckWait - prefs_.audioNotifierDelay || prevAudibleDuration < duckWait) countDown = 0; // If it isn't audible long, then unduck after we know it isn't audible anymore (i.e. two seconds)
  else countDown = unduckWait - timeSinceAudibleBecameFalse;
...
var getPrevAudibleDuration = function getPrevAudibleDuration(tabId) {
  var lastAudibleOrPlayed = getLastAudibleOrPlayed(tabId);
  var lastSilentOrPaused = getLastSilentOrPaused(tabId);
  ...
  var longestAudibleDuration = getState(tabId, 'longestPrevDuration');
...
var audibleTooShort = function audibleTooShort(tabId) {
  // If it ever played and is on the Manual Ducking Controls list, then it will stay unducked until the url is no longer active
  var domainIsInList = prefsStore.domainInList(getState(tabId, 'domainCached'), prefs_.manualduckinglist);
  if (getState(tabId, 'lastAudibleStart').getTime() && domainIsInList) return false;
  ...
  if (isPlayingOrPlayInProgress(tabId)) {
    if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'audibleTooShort1', getState(tabId, 'lastAudibleStart') === 0);
    return getState(tabId, 'lastAudibleStart').getTime() === 0;
  }

  var prevAudibleDuration = getPrevAudibleDuration(tabId);

  if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'audibleTooShort2', prevAudibleDuration, prefs_.minTimeBeforeDucking);

  return prevAudibleDuration < prefs_.minTimeBeforeDucking;
};

var inaudibleTooLong = function inaudibleTooLong(tabId) {
  // console.log(tabId, "countdown=", getCountdownForUnducking(tabId, false));

  return getCountdownForUnducking(tabId, false) <= 0;
};

var wasRecentlyAudible = function wasRecentlyAudible(tabId) {
  var lastAudibleAndUnmutedDelta = new Date().getTime() - getState(tabId, 'lastAudibleAndUnmuted').getTime();
...
var maybeAudible = function maybeAudible(tabId) {
  var tooShort = audibleTooShort(tabId);
  var tooLong = inaudibleTooLong(tabId);
  setState(tabId, '_audibleTooShortCached', tooShort);
  setState(tabId, '_inaudibleTooLongCached', tooLong);
  if (logTypeEnabled_.duckingReasoning) console.log(tabId, 'maybeAudible', !tooShort, !tooLong);

  return !tooShort && !tooLong;
};

var initMusicDucking = function initMusicDucking() {
  if (!prefs_.enableDucking && getFirstDuckedTabId() === null) return;

  stopMusicDucking();
  musicDuckingIntervalId_ = setInterval(function () {
    var checkMaybeAudibleAll = getStateForAllTabs('checkMaybeAudible');
    var shouldUpdateDucking = false;
    var reason = '';
    ...
    var now = new Date();
    var updateFrequently = false;
    Object.keys(checkMaybeAudibleAll).forEach(function (tabId) {
      if (checkMaybeAudibleAll[tabId].checkMaybeAudible === null) return;

      if (checkMaybeAudibleAll[tabId].checkMaybeAudible.updateFrequently) updateFrequently = true;

      // TODO: remove this block if we allow multiple untils
      var until = checkMaybeAudibleAll[tabId].checkMaybeAudible.until;
      var isDate = until instanceof Date && !isNaN(until.valueOf());
      if (!isDate) {
      ...
      if (now.getTime() > until.getTime() || !isDate) {
        ...
        updateDucking(tabId + ' - ducking interval over').done(); // force it to run once at end (TODO: make code less dependent on this)
      }
      ...
      var maybeAudibleResult = maybeAudible(tabId);
      if (maybeAudibleResult !== checkMaybeAudibleAll[tabId].checkMaybeAudible.maybeAudible || tabId === unduckedTabId_ && maybeAudibleResult === false) {
        var checkMaybeAudible = getState(tabId, 'checkMaybeAudible');
    ...
    var promise = shouldUpdateDucking ? updateDucking(reason) : updateCountdownForUnducked();
    promise.done();
  }, prefs_.duckingInterval * 1000);
  console.log('ducking interval started');
};

var stopMusicDucking = function stopMusicDucking() {
  if (!prefs_.enableDucking && getFirstDuckedTabId() === null) return;
...
var addMaybeAudibleCheck = function addMaybeAudibleCheck(tabId, timeToCheck, updateFrequently) {
  if (tabId === windowManager.getExtensionTabIdSync()) return;

  var until = new Date(new Date().getTime() + (1 + timeToCheck) * 1000);
  ...
  */
  ...
  if (musicDuckingIntervalId_ === 0) initMusicDucking();
};

var showDuckingInfoInLog = function showDuckingInfoInLog(reason, showFullInfo) {
  updateDuckingCount_++;
  if (!showFullInfo) {
...
var categorizeTabIdsForUpdateDucking = function categorizeTabIdsForUpdateDucking(tabIds) {
  var state = { needToUnduck: [], needToDuck: [], needToClearDuck: [], unduckable: [], shortTime: [], ducked: [], other: [] };
  tabIds.forEach(function (tabId) {
    var category = categorizeTabIdForUpdateDucking(tabId);
    if (!state.hasOwnProperty(category)) state[category] = [];
...
var categorizeTabIdForUpdateDucking = function categorizeTabIdForUpdateDucking(tabId) {
  var determineDuckedOrOther = function determineDuckedOrOther() {
  ...
  try {
    ...
    if (getState(tabId, 'ducked') && inManualDuckingList) {
      ...
      if (inaudibleTooLong(tabId)) {
        // order between checking toolong and tooshort matters for paused videos
...
var duckingEffectivelyEnabled = function duckingEffectivelyEnabled() {
  return !prefs_.disableAutomuting && !prefs_.privacyMode && prefs_.enableDucking;
};

// Show the table if it changed
var logDuckingTabState = function logDuckingTabState(duckingTabState, reason) {
...
var updateDucking = function updateDucking(reason) {
  if (!prefs_.enableDucking && getFirstDuckedTabId() === null) return Q.when(null);

  var duckingTabState = updateDuckingInternal(reason);

  // Clear, duck, unduck and update UI
  return clearDucked(duckingTabState.needToClearDuck).then(duckTabIds(duckingTabState.needToDuck)).then(unduckTabIds(duckingTabState.needToUnduck)).then(pauseAllUnmutedDucked()).then(muteAllAudiblePausedDucked()).then(updateCountdownForUnducked()).then(updateContextMenus()).then(setFavicon()).then(refreshUi());
};

// Updates unduckedShortTimeTabId_, unduckedTabId_, unduckingOrder_ and returns a duckingTabState
var updateDuckingInternal = function updateDuckingInternal(reason) {
  var tabIds = getAllTabIds();
  var duckingTabState = categorizeTabIdsForUpdateDucking(tabIds);
  ...
  if (duckingEffectivelyEnabled()) {
    ...
    if (duckingTabState.unduckable.length === 1) {
      ...
      if (duckingTabState.shortTime.length === 0) {
        var firstDuckedTabId = getFirstDuckedTabId();
        ...
        if (unduckedShortTimeTabId_ !== -1) unduckedTabId_ = unduckedShortTimeTabId_;
      }
    }

    if (unduckedTabId_ >= 0) {
      // Only unduck if necessary
      if (getState(unduckedTabId_, 'ducked')) duckingTabState.needToUnduck = [unduckedTabId_];
    }

    // Prepare to duck non-best tabids that could be ducked
    duckingTabState.needToDuck = nonBestTabIds.filter(function (tabId) {
...
var muteAllAudiblePausedDucked = function muteAllAudiblePausedDucked() {
  if (prefs_.disableAutomuting) return Q.when(null);

  var mutePromises = [];
  unduckingOrder_.forEach(function (tabId) {
    if (!getState(tabId, 'mutedCached') && getState(tabId, 'audibleCached') && getState(tabId, 'ducked') && getMusicState(tabId, 'isPlaying') === false) {

      var delta = (new Date().getTime() - getState(tabId, 'lastPaused').getTime()) / 1000;

      // assuming that pausing always works; just verify that paused as well.
      if (delta > prefs_.audioNotifierDelay + 1) mutePromises.push(updateMuted(tabId, true, { shouldUpdateDucking: false }, 'Muted by music ducking.'));
    }
  });
  return Q.allSettled(mutePromises);
};

var getBestTabIdAndAppendOthers = function getBestTabIdAndAppendOthers(currentTabId, tabIds, othersArray) {
  if (tabIds.length === 0) {
    return currentTabId;
  }

  var bestTabId = void 0;
  var bestIndex = void 0;
  var bestDict = {};
  ...
  var filter = tabIds.filter(function (tabId) {
    return tabId !== bestTabId;
  });
...
var clearDucked = function clearDucked(tabIdsToClear) {
  var unmutePromises = [];
...
var pauseAllUnmutedDucked = function pauseAllUnmutedDucked() {
  if (prefs_.disableAutomuting) return Q.when(null);

  var injectPromises = [];

  unduckingOrder_.forEach(function (tabId) {
    if (getState(tabId, 'ducked') && !getState(tabId, 'mutedCached')) injectPromises.push(pauseMusic(tabId, 'Paused because tab is ducked.'));
  });
  return Q.allSettled(injectPromises);
};

var getFirstDuckedTabId = function getFirstDuckedTabId() {
  var firstDuckedTabId = null;
  unduckingOrder_.forEach(function (tabId) {
...
var duckTabIds = function duckTabIds(tabIds) {
  return Q.when(null).then(function () {

    if (tabIds.length === 0) return Q.when(null);
    ...
    tabIds.forEach(function (tabId) {
      ...
      var inManualDuckingList = prefsStore.domainInList(getState(tabId, 'domainCached'), prefs_.manualduckinglist);
      var isPlaying = getMusicState(tabId, 'isPlaying');
      var autoplayCountdown = getMusicState(tabId, 'autoplayCountdown') || false;
      if ((isPlaying === '' || isPlaying === null || inManualDuckingList) && !getState(tabId, 'mutedCached') && getState(tabId, 'audibleCached')) {
    ...
    if (mutePromises.length === 0 && injectPromises.length === 0) return Q.when(null);

    return Q.allSettled(injectPromises).then(function () {
      return Q.allSettled(mutePromises).then(function () {
...
var unduckTabIds = function unduckTabIds(tabIds) {
  return Q.when(null).then(function () {
    ...
    tabIds.forEach(function (tabId) {
      ...
      var index = unduckingOrder_.indexOf(tabId);
      if (index > -1) {
        if (!isPlayingOrPlayInProgress(tabId)) injectPromises.push(playMusic(tabId, 'Played while music unducking.'));
        mutePromises.push(updateMuted(tabId, false, { shouldUpdateDucking: false }, 'Unmuted by music unducking.'));
      }
    });

    return Q.allSettled(mutePromises).done(Q.allSettled(injectPromises));
  });
};
...
var removeFromArray = function removeFromArray(array, val) {
...
var addToArray = function addToArray(array, val) {
  var index = array.indexOf(val);
...
var getDomain = function getDomain(url) {
  try {
    ...
    if (url.indexOf('chrome-extension://') === 0) return 'chrome-extension://' + new URL(url).hostname + '/';
...
(function (original) {
  ...
  logTypes_.forEach(function (logType) {
    var logTypeFixedCase = logType.substr(0, 1).toUpperCase() + logType.substr(1);
...
var updateMutingOnLoad = function updateMutingOnLoad() {
...
var loadSettings = function loadSettings() {
  return prefsStore.load().then(function (prefs) {
    prefs_ = prefs;
    return setFavicon().then(afterLoadSettings());
  });
};
...
var afterLoadSettings = function afterLoadSettings() {
  return windowManager.getTabs().then(function (tabs) {
    return afterLoadSettingsFirstTime(tabs).then(function () {
      console.log('afterLoadSettings tabs', tabs);
      var injectMusicPromises = tabs.map(function (tab) {
        return Q.fcall(function () {
          injectMusicApi(tab.id, tab.url, true);
        });
      });
      return Q.allSettled(injectMusicPromises).then(function () {
        unduckingOrder_ = tabs.map(function (tab) {
          return tab.id;
        });
...
var afterLoadSettingsFirstTime = function afterLoadSettingsFirstTime(tabs) {
  if (!isFirstTime_) return Q.when(null);
  ...
  return Q.when(null).then(function () {
    ...
    var maybePrivacyModePromise = Q.when(null);
    if (prefs_.privacyMode) maybePrivacyModePromise = muteEverything();
...
var setupTestScenario = function setupTestScenario(senderTabId, senderWindowId, setupConfig) {
  console.log('setupTestScenario', setupConfig);

  var i = void 0;

  var maybeLaunchIncognitoWindow = Q.when(null);
  if (setupConfig.testUrlsInIncognito || false) {
    maybeLaunchIncognitoWindow = windowManager.createWindow({ incognito: true });
  }

  return maybeLaunchIncognitoWindow.then(function (incognitoWindow) {
    var createdTabIds = [];
    var createdTabIdDict = {};

    var createTabPromises = setupConfig.urls.map(function (url) {
      return windowManager.createTab({ url: url, windowId: senderWindowId });
    });
    ...
    if (setupConfig.testUrlsInIncognito || false) {
      var createTabIncognitoPromises = setupConfig.urls.map(function (url) {
        return windowManager.createTab({ url: url, windowId: incognitoWindow.id });
      });
    ...
    return Q.all(createTabPromises).then(function (tabs) {
      console.log(tabs);
      tabs.forEach(function (tabInfo) {
      ...
      return windowManager.getTabs().then(function (tabs) {
        ...
        var tabIdsToClose = filtered.map(function (tab) {
          return tab.id;
        });
        console.log('tabIdsToClose', tabIdsToClose);

        return chromeMisc.removeTabIds(tabIdsToClose).then(function () {
          var setupInfo = { tabIds: createdTabIds, senderTabId: senderTabId };
          if (setupConfig.skipSwitchBackToTest || false) return Q.when(setupInfo);

          return windowManager.switchToTab(senderTabId).then(function () {
...
var setPropertiesMultiple = function setPropertiesMultiple(tabIds, properties) {
  return Q.fcall(function () {
    var now = new Date();
    var count = void 0,
        index = void 0;

    var setPropertiesPromises = [];
    ...
    if (properties.audible === true) {
      ...
      tabIds.forEach(function (tabId) {
        ...
        if (getState(tabId, 'lastUnmuted')) setState(tabId, 'lastUnmuted', getState(tabId, 'lastAudibleStart'));
        index++;
      });
    }
    ...
    tabIds.forEach(function (tabId) {
      if (properties.muted || false) setPropertiesPromises.push(updateMuted(tabId, true, {}, 'Muted as part of testing scenario.'));
...
windowManager.init().then(loadSettings()).then(function () {
...
},{"./background/chrome_misc":5,"./background/music_controllers":6,"./background/window_manager":7,"./prefs_store":8,"q":2}],5:[function(require,module,exports){
"use strict";
...
},{"../util":9,"./chrome_misc":5,"q":2}],8:[function(require,module,exports){
"use strict";
...
},{"./util":9,"q":2}],9:[function(require,module,exports){
"use strict";
