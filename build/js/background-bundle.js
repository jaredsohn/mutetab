...
if(changeInfo.hasOwnProperty('mutedInfo')){if(tab.audible&&tab.mutedInfo.muted){setState(tabId,'lastAudibleAndUnmuted',new Date());refreshUi();}if(changeInfo.mutedInfo.extensionId!==chrome.runtime.id){if(logTypeEnabled_.events)console.log('onUpdated',tabId,tab.url,changeInfo,tab);var mutedCauseText=changeInfo.mutedInfo.reason;if(mutedCauseText==='extension')mutedCauseText='another extension. (id='+changeInfo.mutedInfo.extensionId+')';else if(mutedCauseText==='user')mutedCauseText='user through Chrome.';else mutedCauseText+='.';updateMuted(tabId,changeInfo.mutedInfo.muted,{saveNonPrivate:true},(tab.mutedInfo.muted?'Muted by ':'Unmuted by ')+mutedCauseText).done();}}};var onActivated=function onActivated(activeInfo){console.log("onActivated",activeInfo.windowId,windowManager.getExtensionWindowIdSync());if(activeInfo.windowId===windowManager.getExtensionWindowIdSync())return;moveToFrontOfUnduckingOrder(activeInfo.tabId); // if (logTypeEnabled_.events)
//   console.log(activeInfo.tabId, "onActivated");
var prevTabId=windowManager.getLastTabIdSync();windowManager.setLastWindowId(activeInfo.windowId);windowManager.setLastTabId(activeInfo.tabId);windowManager.getTabInfo(activeInfo.tabId).then(function(tabInfo){setState(activeInfo.tabId,'domainCached',getDomain(tabInfo.url)); // console.log("just tabbed away from (will mute if not music)", prevTabId);
...
if(domain!==null){var wasMuted=getState(tab.id,'mutedCached'); //console.log("~~~", prefs_.mutedRememberSameDomain, (domain === oldDomain), (!prefs_.disableAutomuting), domain, oldDomain);
...
// Load Settings
////////////////////////////////////////////////////////////////////////
var loadSettings=function loadSettings(){return prefsStore.load().then(function(prefs){prefs_=prefs;return setFavicon().then(afterLoadSettings());});}; // Every time after loading settings we:
