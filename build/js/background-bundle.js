...
//  }
};var updateListForCurrentTab=function updateListForCurrentTab(listType){return windowManager.getCurrentTab().then(function(currentTabInfo){if(!currentTabInfo){return Q.when(null);}var domainToUpdate=getDomain(currentTabInfo.url);if(listType=='white'||listType=='black'){var blackOrWhiteListDomain=prefsStore.getDomainRuleForDomainInList(domainToUpdate,prefs_[listType+'list']);if(blackOrWhiteListDomain!==null)domainToUpdate=blackOrWhiteListDomain;}return prefsStore.updateListAndSave(prefs_,listType,domainToUpdate).then(updateContextMenus()).done();});};////////////////////////////////////////////////////////////////////////
...
var muteInfo=null;var domain=getDomain(tab.url);var oldDomain=getState(tab.id,'domainCached');var oldDucked=getState(tab.id,'ducked');var oldAudible=getState(tab.id,'audibleCached');var oldAudibleStart=getState(tab.id,'lastAudibleStart');// console.log("domain: ", domain, getState(tab.id, "domainCached"));
...
var updateContextMenus=function updateContextMenus(){clearTimeout(updateContextMenusTimeout_);updateContextMenusTimeout_=setTimeout(function(){updateContextMenusTimeout_=null;return windowManager.getCurrentTab().then(function(currentTabInfo){if(!currentTabInfo){return Q.when(null);}// console.log("updateContextMenus", currentTabInfo.id, currentTabInfo.url, unduckedTabId_);
